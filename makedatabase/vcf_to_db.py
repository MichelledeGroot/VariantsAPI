...
def main():
    ...
    logger = logging.getLogger("vcf_to_db")
    variants = []
    for file in os.listdir("Resources"):
        try:
            new_variants = vcf_reader.file_to_vcfs(os.path.join("Resources", file))
            logger.info(str(len(new_variants))+" new possible pathogenic variants in file: "+file)
            variants += new_variants
        except FileNotFoundError:
            print("File not found: " + file)
    logger.info("Found : "+ str(len(variants))+" possible pathogenic variants from VCF files")
